...
export default function GameRoom() {
  ...
  const roomQuery = createRoom(roomId)
  ...
  const handleStartGame = async () => {
    await startGameMutation.mutateAsync({ roomId: roomId() })
    await roomQuery.refetch()

    if (isMilito()) {
      await initMilitoMutation.mutateAsync({ roomId: roomId() })
      await militoQuery.refetch()
    } else {
      await initGameMutation.mutateAsync({ roomId: roomId() })
  ...
  createEffect(() => {
    const gameData = game()
    if (!gameData?.state || isMilito()) return

    const { flippedIndices, cards } = gameData.state
    ...
    if (flippedIndices.length === 2) {
      ...
      if (card1.value !== card2.value) {
        const timer = window.setTimeout(() => {
          resetFlippedMutation.mutate({ roomId: roomId() })
        }, 2000)
  ...
  const handleCardClick = (cardId: number) => {
    ...
    const { flippedIndices, currentPlayerPosition } = gameData.state

    if (currentPlayerPosition !== gameData.currentPlayerPosition) {
      return
    }
    ...
    flipCardMutation.mutate({ roomId: roomId(), cardId })
  }
  ...
  const handleMilitoSelectCard = (cardIndex: number) => {
    selectCardMutation.mutate({ roomId: roomId(), cardIndex })
  }

  const handleMilitoSelectColumn = (columnIndex: number) => {
    selectColumnMutation.mutate({ roomId: roomId(), columnIndex })
  }

  const handleMilitoDiscard = (cardIndex: number) => {
    discardMutation.mutate({ roomId: roomId(), cardIndex })
  }
            ...
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-gray-400">
                  ...
                  <Show when={room()!.status === 'playing'}>
                    Game in progress
                  </Show>
                  <Show when={room()!.status === 'finished'}>
                    Game finished!
                  </Show>
                </p>
              </div>
            ...
            <div>
              ...
              <div class="mb-4 p-4 bg-cyan-500/20 border border-cyan-500/50 rounded-xl">
                <p class="text-cyan-200 text-center">
                  <Show
                    when={game()!.state.currentPlayerPosition === game()!.currentPlayerPosition}
                    fallback={`Player ${game()!.state.currentPlayerPosition + 1}'s turn`}
                  >
                    It's your turn!
                  </Show>
                </p>
